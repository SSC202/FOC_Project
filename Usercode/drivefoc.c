...
void drive_foc_calc(void)
{
     ...
     */
    // i_alpha_nh_1 = Drive_hfi.i_alpha_beta_n_h.alpha * sinf(2 * Drive_hfi.theta_obs - Drive_hfi.phase_h);
    // i_alpha_nh_2 = Drive_hfi.i_alpha_beta_n_h.alpha * cosf(2 * Drive_hfi.theta_obs - Drive_hfi.phase_h);
    // i_alpha_nh_1_filter.input = i_alpha_nh_1;
    // i_alpha_nh_2_filter.input = i_alpha_nh_2;
    // LPF_Calc(&i_alpha_nh_1_filter, system_enable);
    // LPF_Calc(&i_alpha_nh_2_filter, system_enable);
    ...
    // i_beta_nh_1 = Drive_hfi.i_alpha_beta_n_h.beta * sinf(2 * Drive_hfi.theta_obs - Drive_hfi.phase_h);
    // i_beta_nh_2 = Drive_hfi.i_alpha_beta_n_h.beta * cosf(2 * Drive_hfi.theta_obs - Drive_hfi.phase_h);
    // i_beta_nh_1_filter.input = i_beta_nh_1;
    // i_beta_nh_2_filter.input = i_beta_nh_2;
    // LPF_Calc(&i_beta_nh_1_filter, system_enable);
    // LPF_Calc(&i_beta_nh_2_filter, system_enable);
    ...
    if (system_dac_print == 0) {
        hwdac_value1 = (uint16_t)(32768);
        hwdac_value2 = (uint16_t)(32768);
    } else if (system_dac_print == 1) {
        hwdac_value1 = (uint16_t)((Drive_hfi.theta_true + M_PI) / (2 * M_PI) * 65535);
        hwdac_value2 = (uint16_t)((Drive_hfi.theta_obs + M_PI) / (2 * M_PI) * 65535);
        hwdac_value3 = (uint16_t)((Drive_hfi.theta_error + M_PI) / (2 * M_PI) * 65535);
    } else if (system_dac_print == 2) {
        hwdac_value1 = (uint16_t)((Drive_hfi.speed_true + 150.0f) / (300.0f) * 65535);
        hwdac_value2 = (uint16_t)((Drive_hfi.speed_obs + 150.0f) / (300.0f) * 65535);
        hwdac_value3 = (uint16_t)((Drive_hfi.speed_error + 150.0f) / (300.0f) * 65535);
    } else if (system_dac_print == 3) {
        hwdac_value1 = (uint16_t)((Drive_ualphabetah.alpha + 200.0f) / (400.0f) * 65535);
        hwdac_value2 = (uint16_t)((Drive_ualphabetah.beta + 200.0f) / (400.0f) * 65535);
    } else if (system_dac_print == 4) {
        hwdac_value1 = (uint16_t)((Drive_ialphabeta.alpha + 4.5f) / (9.0f) * 65535);
        hwdac_value2 = (uint16_t)((Drive_ialphabeta.beta + 4.5f) / (9.0f) * 65535);
    } else if (system_dac_print == 5) {
        hwdac_value1 = (uint16_t)((Drive_hfi.i_alpha_beta_h.alpha + 0.25f) / (0.5f) * 65535);
        hwdac_value2 = (uint16_t)((Drive_hfi.i_alpha_beta_h.beta + 0.25f) / (0.5f) * 65535);
    } else if (system_dac_print == 6) {
        hwdac_value1 = (uint16_t)((Drive_idq.d + 4.5f) / (9.0f) * 65535);
        hwdac_value2 = (uint16_t)((Drive_idq.q + 4.5f) / (9.0f) * 65535);
    } else if (system_dac_print == 7) {
        hwdac_value1 = (uint16_t)((Drive_hfi.error_dem + 0.25f) / (0.5f) * 65535);
    }
}