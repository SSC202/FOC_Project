...
{
    alpha_beta->alpha = (2.f / 3.f) * ((1.f) * abc->a - (0.5f) * abc->b - (0.5f) * abc->c);
    alpha_beta->beta  = (2.f / 3.f) * ((0.f) * abc->a + (sqrtf(3.f) / 2.f) * abc->b - (sqrtf(3.f) / 2.f) * abc->c);
}
...
void alphabeta_2_abc(alpha_beta_t *alpha_beta, abc_t *abc)
{
    ...
    abc->b = (-(0.5f) * alpha_beta->alpha + (sqrtf(3.f) / 2.f) * alpha_beta->beta);
    abc->c = (-(0.5f) * alpha_beta->alpha - (sqrtf(3.f) / 2.f) * alpha_beta->beta);
}
